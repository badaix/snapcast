...
PcmDevice getPcmDevice(const std::string& player, const std::string& parameter, const std::string& soundcard)
{
    ...
    std::vector<PcmDevice> pcm_devices;
#if defined(HAS_ALSA)
...
#pragma message "Warning: the macOS support is experimental and might not be maintained"
    ...
    try
    {
        ...
        std::string pcm_device(player::DEFAULT_DEVICE);
        ...
        op.add<Value<std::string>>("h", "host", "server hostname or ip address", "", &settings.server.host);
        ...
        op.add<Value<std::string>>("", "hostID", "unique host id, default is MAC address", "", &settings.host_id);

// PCM device specific
#if defined(HAS_ALSA) || defined(HAS_PULSE) || defined(HAS_WASAPI)
        auto listSwitch = op.add<Switch>("l", "list", "list PCM devices");
        /*auto soundcardValue =*/op.add<Value<std::string>>("s", "soundcard", "index or name of the pcm device", pcm_device, &pcm_device);
#endif
        /*auto latencyValue =*/op.add<Value<int>>("", "latency", "latency of the PCM device", 0, &settings.player.latency);
#ifdef HAS_SOXR
        auto sample_format = op.add<Value<std::string>>("", "sampleformat", "resample audio stream to <rate>:<bits>:<channels>", "");
#endif

        auto supported_players = Controller::getSupportedPlayerNames();
        std::string supported_players_str;
        for (const auto& supported_player : supported_players)
            supported_players_str += (!supported_players_str.empty() ? "|" : "") + supported_player;
        op.add<Value<std::string>>("", "player", supported_players_str + "[:<options>|?]", supported_players.front(), &settings.player.player_name);
...
// sharing mode
        ...
        if (hw_mixer_supported)
            mixer_mode = op.add<Value<std::string>>("", "mixer", "software|hardware|script|none|?[:<options>]", "software");
        else
            mixer_mode = op.add<Value<std::string>>("", "mixer", "software|script|none|?[:<options>]", "software");
...
// daemon settings
        ...
        auto userValue = op.add<Value<std::string>>("", "user", "the user[:group] to run snapclient as when daemonized");
#endif

        // logging
        op.add<Value<std::string>>("", "logsink", "log sink [null,system,stdout,stderr,file:<filename>]", settings.logging.sink, &settings.logging.sink);
        auto logfilterOption = op.add<Value<std::string>>(
        ...
        if (daemonOption->is_set())
        {
            ...
            if (userValue->is_set())
            {
                ...
                std::vector<std::string> user_group = utils::string::split(userValue->value(), ':');
                user = user_group[0];
        ...
        std::string mode = utils::string::split_left(mixer_mode->value(), ':', settings.player.mixer.parameter);
        if (mode == "software")
        ...
        {
            std::cout << "mixer can be one of 'software', " << (hw_mixer_supported ? "'hardware', " : "") << "'script', 'none'\n"
        ...
        auto controller = std::make_shared<Controller>(io_context, settings);
        controller->start();
    ...
    LOG(NOTICE, LOG_TAG) << "Snapclient terminated." << std::endl;
    exit(exitcode);
}
