...
Snapserver supports [librespot](https://github.com/librespot-org/librespot) with the `pipe` backend.
 ...
 2. Configure snapserver with `stream = spotify:///librespot?name=Spotify[&username=<my username>&password=<my password>][&devicename=Snapcast][&bitrate=320][&onstart=<start command>][&onstop=<stop command>][&volume=<volume in percent>][&volumectrl=log][&cache=<cache dir>][&disable_audio_cache=false][&killall=true]`
    * Valid bitrates are 96, 160, 320
    * Valid volumecontrol values are linear, log, fixed. Choose between a logarithmic (default), linear, or fixed volume scaling.
