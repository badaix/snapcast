...
class LibrespotControl(object):
    ...
    def getMetaData(self, data):
        ...
        if 'shuffle_context' in data:
            self._properties['shuffle'] = data['shuffle_context']
        if 'repeat_context' in data and 'repeat_track' in data:
            if data['repeat_track']:
                self._properties['loopStatus'] = 'track'
            elif data['repeat_context']:
        ...
        if 'track' in data:
            ...
            if 'artist_names' in track:
                ...
                for artist in artists:
                    metadata['artist'].append(artist)
            if 'album_name' in track:
                metadata['album'] = track['album_name']
            if 'album_cover_url' in track:
                metadata['artUrl'] = track['album_cover_url']
            if 'release_date' in track:
                date = {k: int(v) for k, v in (item.split(":") for item in track['release_date'].split())}
                metadata['contentCreated'] = f"{date['year']:04d}-{date['month']:02d}-{date['day']:02d}"
            if 'disc_number' in track:
                metadata['discNumber'] = track['disc_number']
            if 'track_number' in track:
                metadata['trackNumber'] = track['track_number']
            if 'position' in track:
                self._properties['position'] = max(0.0, float(track['position']) / 1000.0)
            metadata['trackId'] = track['uri']

        return metadata

    def updateProperties(self):
        res = self.send_request('status', r_method="GET")
        if res.status_code / 100 == 2:
            self._metadata = self.getMetaData(json.loads(res.text))
        ...
        # Notification
            ...
            if 'type' in jmsg:
                ...
                if event == 'stopped':
                    ...
                    self._properties['volume'] = int(jmsg['data']['value'])
                elif event == 'seek':
                    self._properties['position'] = float(jmsg['data']['position']) / 1000.0
                elif event == 'shuffle_context':
                    self._properties['shuffle'] = jmsg['data']['value']
                elif event == 'repeat_context':
                    self.updateProperties()
                elif event == 'repeat_track':
                    self.updateProperties()
                elif event == 'metadata':
                    self.updateProperties()
                else:
    ...
    def send_request(self, method, params=None, r_method="POST"):
        request = f"http://{self._params['librespot-host']}:{self._params['librespot-port']}/{method}"
        logger.debug(f"Sending request: {request}")
        r = requests.request(r_method, request, json=params)
        logger.info(
        ...
        try:
            ...
            if interface == 'Plugin.Stream.Player':
                if cmd == 'Control':
                    ...
                    if command == 'next':
                        self.send_request("player/next", params={})
                        ...
                        self.send_request(
                            "player/seek", {"position": int(position * 1000)})
                    elif command == 'seek':
                    ...
                    if 'shuffle' in property:
                        ...
                        self.send_request("player/shuffle_context", params={'shuffle_context': self._properties['shuffle']})
                    if 'loopStatus' in property:
                        value = property['loopStatus']
                        self._properties['loopStatus'] = value
                        if value == "playlist":
                            self.send_request(
                                "player/repeat_context", {"repeat_context": True})
                        elif value == "track":
                            self.send_request(
                                "player/repeat_track", {"repeat_track": True})
                        elif value == "none":
                            self.send_request(
                                "player/repeat_context", {"repeat_context": False})
                            self.send_request(
                                "player/repeat_track", {"repeat_track": False})
                    if 'volume' in property:
                        self.send_request(
                            "player/volume", {"value": property['volume']})
                    if 'mute' in property:
                        logger.debug("todo: not implemented")
                        # self._properties['mute'] = False
